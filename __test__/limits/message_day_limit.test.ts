...
describe('Максимальная длина сообщений', () => {
    ...
    let oneUserRoomID = ""
    let twoUsersRoomID = ""
    let publicMessagesCount = config.userLimits.messages.publicMessagesInDay
    let firstPublicRoomMessageCount = 10
    let secondPublicRoomMessageCount = publicMessagesCount - firstPublicRoomMessageCount
    let privateMessagesCount = config.userLimits.messages.privateMessagesInDay
    let firstPrivateRoomMessageCount = 10
    let secondPrivateRoomMessageCount = privateMessagesCount - firstPrivateRoomMessageCount
    ...
    describe('Успешно отправляем сообщения', () => {
        it(`Отправляем ${firstPublicRoomMessageCount} сообщение в комнату (с паролем)`, async () => {
            // const temp = async () => {
            for (let i = 0; i < firstPublicRoomMessageCount; ++i) {
        ...
        it(`Отправляем ${secondPublicRoomMessageCount} сообщение в комнату (без пароля)`, async () => {
            // const temp = async () => {
            for (let i = 0; i < secondPublicRoomMessageCount; ++i) {
    ...
    describe('Превышаем лимит', () => {
        it('Отправляем сообщение в комнату (с паролем)', async () => {
            ...
            expect(res.body.message).toEqual(`Достигнут дневной лимит количества сообщений для данного типа комнат`)
            // }
        });
    ...
    describe('Успешно отправляем сообщения', () => {
        it(`Отправляем ${firstPrivateRoomMessageCount} сообщений самому себе`, async () => {
            // const temp = async () => {
            for (let i = 0; i < firstPrivateRoomMessageCount; ++i) {
                const res = await successSendMessage(cookie__value__user1, oneUserRoomID, user1.id, 'Т'.repeat(config.userLimits.messages.maxLength))
                expect(res.recipients).toEqual([user1.id])
            }
            // }
        });
    ...
    describe('Успешно отправляем сообщения', () => {
        it(`Отправляем ${secondPrivateRoomMessageCount} сообщений в приватный чат`, async () => {
            for (let i = 0; i < secondPrivateRoomMessageCount; ++i) {
                const res = await successSendMessage(cookie__value__user1, twoUsersRoomID, user1.id, 'Т'.repeat(config.userLimits.messages.maxLength))
            }
        });
    })
