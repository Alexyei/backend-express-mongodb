...
describe('Отправка сообщений в приватной комнате', () => {
    ...
    describe('Валидация отправки сообщений в комнату в которой не состоим', () => {
        ...
        it('Успешное создание приватной комнаты', async () => {
            const res = await request.post('/api/private-rooms/create').set('Cookie', `${cookie__value__user1};`).send({"login": "user3"})
            expect(res.status).toEqual(200)
            const room: PrivateRoomWithMessagesDTO = res.body
            roomID = room.id;
        })
    ...
    describe('Проверяем сообщения второго пользователя', () => {
        it('Проверяем сообщения для одной комнаты', async () => {
            const data = await successCheckPrivateRoomMessage(cookie__value__user2, roomID);
            expect(data.messages.length).toEqual(2)
            data.messages.forEach((m) => expect(m.author.login).toEqual(user1.login))
            data.messages.forEach((m) => expect(m.message.includes('Первый')).toEqual(true))
        })
    ...
    describe('Успешно отправляем сообщения вторым пользователем', () => {
        it('Отправляем сообщение в комнату', async () => {
            const res = await successSendMessage(cookie__value__user2, roomID, user2.id, 'Второй 1')
            expect(res.recipients.includes(user1.id)).toEqual(true)
            expect(res.recipients.includes(user2.id)).toEqual(true)
        });

        it('Повторно отправляем сообщение в комнату', async () => {
            const res = await successSendMessage(cookie__value__user2, roomID, user2.id, 'Второй 2')
            expect(res.recipients.includes(user1.id)).toEqual(true)
            expect(res.recipients.includes(user2.id)).toEqual(true)
        });
    ...
    describe('Проверяем сообщения первого пользователя', () => {
        it('Проверяем сообщения для одной комнаты', async () => {
            ...
            expect(data.messages.some(m => m.author.login == user1.login)).toEqual(true);
            expect(data.messages.some(m => m.author.login == user2.login)).toEqual(true);
            expect(data.messages.some(m => m.message.includes('Первый'))).toEqual(true);
            expect(data.messages.some(m => m.message.includes('Второй'))).toEqual(true);
        })

        it('Проверяем сообщения для всех комнат', async () => {
            const data = await successCheckPrivateRoomMessages(cookie__value__user1);
            expect(data.length).toEqual(2)
            const logins = data[0].messages.map(m=>m.author.login)
            const res = logins.some(l=>l == user1.login)
            data.forEach(el => el.messages.length && expect(el.messages.some(m => m.author.login == user1.login)).toEqual(true))
            data.forEach(el => el.messages.length && expect(el.messages.some(m => m.author.login == user2.login)).toEqual(true))
        })
    })
